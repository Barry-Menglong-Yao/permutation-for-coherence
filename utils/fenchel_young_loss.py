...
class PerturbedFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input_tensor, y_true, batched, maximize, *args):
        #input_tensor = torch.argsort(torch.argsort(input_tensor))
        diff = input_tensor - y_true
        if not maximize:
            diff = -diff
...
class FenchelYoungLoss(nn.Module):
    ...
    def forward(self, input_tensor, y_true, *args):
        return PerturbedFunc.apply(input_tensor, y_true, self._batched, self._maximize, *args)